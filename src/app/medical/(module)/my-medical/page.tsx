...
export default async function MyMedicalPage() {
  ...
  const [
    ...
    getUserModuleSubscriptionInfo(supabase, user.id, 'medical'),
    supabase.from('courses').select('id').eq('module', 'medical').eq('published', true),
    supabase.from('course_progress').select('course_id').eq('user_id', user.id).eq('completed', true),
    supabase.from('community_posts').select('id', { count: 'exact', head: true }).eq('module', 'medical'),
  ])
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-primary">My Medical</h1>
        <p className="text-secondary text-sm mt-1">Your medical overview at a glance</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <SummaryCard
          ...
          subtitle="Total posts in medical feed"
          icon={Users}
          href="/medical/community"
        />
        <SummaryCard
          title="AI Queries"
