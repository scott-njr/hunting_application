...
export default async function FishingModuleLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: member }, moduleTier] = await Promise.all([
    supabase
      ...
      .eq('id', user.id)
      .maybeSingle(),
    getUserModuleTier(supabase, user.id, 'fishing'),
  ])

  if (moduleTier === 'free') redirect('/pricing?upgrade=basic&module=fishing')

  return (
    <div className="min-h-dvh bg-base text-primary flex flex-col">
      <ModuleHeader userId={user.id} email={user.email ?? ''} messagesHref="/fishing/community/messages" />
