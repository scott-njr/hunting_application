...
export default async function ArcheryModuleLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ data: member }, moduleTier] = await Promise.all([
    supabase
      ...
      .eq('id', user.id)
      .maybeSingle(),
    getUserModuleTier(supabase, user.id, 'archery'),
  ])

  if (moduleTier === 'free') redirect('/pricing?upgrade=basic&module=archery')

  return (
    <div className="min-h-dvh bg-base text-primary flex flex-col">
      <ModuleHeader userId={user.id} email={user.email ?? ''} messagesHref="/archery/community/messages" />
