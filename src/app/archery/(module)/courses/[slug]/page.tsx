...
import { CourseMarkdown } from '@/app/hunting/(module)/courses/[slug]/course-markdown'
import { MarkCompleteButton } from '@/app/hunting/(module)/courses/[slug]/mark-complete-button'

export default async function ArcheryCourseDetailPage({
  params,
}: {
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 text-xs text-muted mb-6">
        <Link href="/archery/courses" className="hover:text-secondary transition-colors">
          Courses
        </Link>
      ...
      <div className="bg-surface border border-subtle rounded-xl p-6 mb-6">
        <div className="flex items-center gap-2 mb-3">
          <span className="bg-teal-900/30 text-teal-400 text-xs font-bold px-2.5 py-1 rounded-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-muted text-xs flex items-center gap-1.5">
            {course.content_type === 'article' ? <BookOpen className="h-3.5 w-3.5" /> : <Video className="h-3.5 w-3.5" />}
        ...
        {totalInCategory > 1 && (
          <div className="flex items-center gap-1.5 mt-4 pt-4 border-t border-subtle">
            {siblingList.map((s, i) => (
              <Link
                key={s.slug}
                href={`/archery/courses/${s.slug}`}
                title={s.title}
                className={`h-2 rounded-full transition-all ${
                  i === currentIdx
                    ? 'w-6 bg-teal-500'
                    : 'w-2 bg-elevated hover:bg-teal-500/40'
                }`}
              />
            ))}
          </div>
        ...
        <>
          ...
          <div className="mt-6 bg-surface border border-subtle rounded-xl px-5 py-4">
            <div className="flex items-center justify-between">
              <MarkCompleteButton courseId={course.id} initialCompleted={isCompleted} />
              <div className="flex items-center gap-2">
                {prevCourse && (
                  <Link
                    href={`/archery/courses/${prevCourse.slug}`}
                ...
                {nextCourse && (
                  <Link
                    href={`/archery/courses/${nextCourse.slug}`}
                    className="flex items-center gap-1.5 text-teal-400 hover:text-teal-300 text-xs font-medium transition-colors px-3 py-1.5 rounded-lg bg-teal-900/20 hover:bg-teal-900/30"
                  >
                    Next Lesson
          ...
          <div className="mt-4 text-center">
            <Link href="/archery/courses" className="text-muted hover:text-secondary text-xs transition-colors">
